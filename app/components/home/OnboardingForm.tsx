...
export default function OnboardingForm() {
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!repo.trim()) return;
    const params = new URLSearchParams({ repo: repo.trim(), role, level });
  ...
  return (
    <div className="w-full max-w-2xl flex flex-col gap-4">
      {/* GitHub URL input */}
      <div className="flex items-center gap-3 bg-white border border-gray-200 rounded-2xl px-5 py-4 shadow-sm focus-within:ring-2 focus-within:ring-indigo-400 focus-within:border-transparent transition">
        {/* GitHub icon */}
        <svg
      ...
      <button
        ...
        className="w-full py-4 rounded-2xl text-sm font-bold text-white bg-indigo-600 hover:bg-indigo-700 active:scale-[0.98] transition-all shadow-md"
      >
        Generate My Kit →
      </button>
